...
const CFG = Object.freeze({
  maxBlocks: 4, clickTimeoutMs: 10_000, betweenClicksMs: 900,
  panelReadyMs: 3000, clipFreshMs: 1800, pollMs: 80,
});
...
const nowTS = () => { const d=new Date(),p=n=>String(n).padStart(2,"0"); return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}:${p(d.getSeconds())}`; };
...
const waitFor = async (fn, to)=>{ const t0=Date.now(); while(Date.now()-t0<to){ try{ if(await fn()) return true; }catch{} await sleep(160);} return false; };
const readClipFresh = async (prev, timeout=CFG.clipFreshMs)=>{ const end=Date.now()+timeout; while(Date.now()<end){ const cur=clipboard.readText().trim(); if(cur && cur!==prev) return cur; await sleep(CFG.pollMs);} return ""; };
...
class LeadStore {
  constructor(dir){
    ...
    this.rows=[]; this.serial=1; this.logKeys=new Set(); this.idIndex=new Map(); this._flushTimer=0;
    this._loadSync();
  }
  _loadSync(){
    try { const j = JSON.parse(fs.readFileSync(this.jsonFile, "utf8")); if (Array.isArray(j)) this.rows = j; } catch {}
    for (let i=0;i<this.rows.length;i++){
      ...
      const ik=idKeyOf(r); if(!this.idIndex.has(ik)) this.idIndex.set(ik,i);
      if(typeof r.notified!=="boolean") r.notified=false;
      if(typeof r.lastSig!=="string") r.lastSig="";
    }
  }
  _debouncedFlush(){ clearTimeout(this._flushTimer); this._flushTimer=setTimeout(()=>this.flush().catch(()=>{}),120); }
  async flush(){ await writeAtomic(this.jsonFile, JSON.stringify(this.rows, null, 2), true); }
  mergeFill(dst, src){
    const F=["product","buyer","mobile","email","company","gstin","address","time"], changed=[];
    for(const f of F){ const cur=String(dst[f]??""), inc=String(src[f]??""); if((cur.trim()===""||cur==="---") && inc.trim()!==""){ dst[f]=inc; changed.push(f);} }
    return changed;
  }
  upsert(row){
    const ts=nowTS(), ik=idKeyOf(row);
    if(this.idIndex.has(ik)){
      // Guard against any stale indices from historical runs
      ...
      if (idx >= 0) {
        ...
        if(changed.length){ this.rows[idx].timestamp=this.rows[idx].timestamp||ts; this._debouncedFlush(); return {action:"merge", index:idx, changedFields:changed}; }
        return {action:"dup", index:idx};
      }
      // If we couldn't locate the index, fall-through to insert as new.
    }
    if(this.logKeys.has(logKeyOf(row))) return {action:"dup", index:-1};
    const rec={ serial:this.serial++, timestamp:ts,
      product:row.product||"", buyer:row.buyer||"", mobile:row.mobile||"",
      email:row.email||"", company:row.company||"", gstin:row.gstin||"",
      address:row.address||"", time:row.time||"", notified:false, lastSig:"" };
    // IMPORTANT: bump existing indices because we'll unshift at the front
    for (const [k, v] of this.idIndex) this.idIndex.set(k, v + 1);
    this.rows.unshift(rec); this.logKeys.add(logKeyOf(row)); this.idIndex.set(ik,0); this._debouncedFlush();
    return {action:"new", index:0};
  }
  get(i){return this.rows[i]}
  markSig(i,sig){ const r=this.rows[i]; if(r){ r.notified=true; r.lastSig=sig; this._debouncedFlush(); } }
}

async function readTextFields(win, specs){
  const code = `(function(S){const $=s=>document.querySelector(s), xp=s=>document.evaluate(s,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue, txt=n=>n?(n.textContent||"").trim().replace(/\\s+/g," "):""; const o={}; for(const f of S){ let v=f.textCss?txt($(f.textCss)):""; if(!v&&f.textXp) v=txt(xp(f.textXp)); o[f.key]=v||'---'; } return o;})(${JSON.stringify(specs)})`;
  return win.webContents.executeJavaScript(code,true);
}
async function clickAny(win, css, xp, scopeSel){
  const code = `(function(css,xp,scope){const root=scope?document.querySelector(scope):document; if(!root) return false; const vis=e=>e&&e.getBoundingClientRect().width>0&&e.getBoundingClientRect().height>0; const tap=e=>{try{e.scrollIntoView({block:'center',inline:'center'})}catch{}; e.dispatchEvent?.(new MouseEvent('click',{bubbles:true})); e.click?.(); return true;}; try{ if(css){ const el=(scope? root.querySelector(css.startsWith(':scope')?css:(':scope '+css)) : document.querySelector(css)); if(vis(el)) return tap(el.ownerSVGElement||el);} }catch{} try{ if(xp){ const el=document.evaluate(xp,root,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue; if(vis(el)) return tap(el.ownerSVGElement||el);} }catch{} return false;})(${JSON.stringify(css)},${JSON.stringify(xp)},${JSON.stringify(scopeSel)})`;
  return win.webContents.executeJavaScript(code,true);
}
...
const notifSig = (r)=> JSON.stringify({ product:r.product||"", buyer:r.buyer||"", company:r.company||"", m:last10(r.mobile), email:r.email||"", gstin:r.gstin||"", address:r.address||"", time:r.time||"" });

function createMessageCentre(opts = {}){
  const {
    log=()=>{}, url="https://seller.indiamart.com/messagecentre/", parent=null,
    windowOptions={ title:"Message Centre", width:1200, height:800, show:false, backgroundColor:"#0f0f10", autoHideMenuBar:true, webPreferences:{ contextIsolation:true, backgroundThrottling:false } },
    maxBlocks=CFG.maxBlocks, clickTimeoutMs=CFG.clickTimeoutMs, betweenClicksMs=CFG.betweenClicksMs, panelReadyTimeoutMs=CFG.panelReadyMs,
    readFreshMs=CFG.clipFreshMs, pollStepMs=CFG.pollMs, autoClose=true, send=async()=>{}
  } = opts;
  ...
  async function readProductFromList(win,i){
    const k=i+1, cssSel=`#splitViewContactList > div > div > div > div:nth-child(${k}) > div > div:nth-child(4) > div.wrd_elip.fl.fs12.fwb.mxwdt75.bgF0F0F0.pd5_20.brdr_rad15 > span`,
          xpSel =`//*[@id="splitViewContactList"]/div/div/div/div[${k}]/div/div[4]/div[1]/span`;
    const code=`(function(css,xp){const t=n=>n?(n.textContent||"").trim().replace(/\\s+/g," "):"";try{const el=document.querySelector(css);if(el)return t(el);}catch{}try{const n=document.evaluate(xp,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;if(n)return t(n);}catch{}return "";} )(${JSON.stringify(cssSel)},${JSON.stringify(xpSel)})`;
  ...
  async function openOnce(){
    ...
    const selFor=(i)=>{ const k=i+1; return [
    ...
    try{
      ...
      for(let i=maxBlocks-1;i>=0;i--){
        let ok=false; for(const [css,xp] of selFor(i)){ ok = await waitFor(()=>clickAny(win,css,xp), clickTimeoutMs); if(ok) break; }
        if(!ok){ log("info",`MC: contact ${i} not available`); continue; }
        ...
        const fromList = await readProductFromList(win, i); if(fromList) base.product = fromList;

        const prev = clipboard.readText().trim();
        ...
        for(const f of BOXES){
          ...
          let val=""; if(clicked){ await sleep(120); val = await readClipFresh(prev, readFreshMs); }
          if(!val) val = await win.webContents.executeJavaScript(`(s=>{const b=document.querySelector(s);return b?(b.textContent||"").trim():"";})(${JSON.stringify(f.container)})`, true);
          ...
          else if (f.kind==="company"){ if(pickGSTIN(val)||isEmail(val)) val=""; val = String(val||"").trim().replace(/\s+/g," "); }
...
try { clipboard.writeText(prev); } catch {}
  ...
  let running=false; const q=[];
  function enqueue(meta={}){ return new Promise((resolve,reject)=>{ q.push({meta,resolve,reject}); if(!running) drain(); }); }
  async function drain(){ running=true; while(q.length){ const job=q.shift(); try{ job.resolve(await openOnce()); }catch(e){ job.reject(e); } } running=false; }

  return { enqueue, get running(){ return running; }, get size(){ return q.length; } };
}
...
function resetLogs(){
  ...
  try { fs.writeFileSync(path.resolve(__dirname, 'Reports/messagecentre_log.json'), '[]', 'utf8'); } catch(e){}
  try { fs.writeFileSync(path.resolve(__dirname, './Reports/messagecentre_log.json'), '[]', 'utf8'); } catch(e){}
